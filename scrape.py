...
def get_ritual_classes(start, end):
    ...
    locations = [
        {"location_num": 1, "name": "Ritual SoMa", "address": "1122 Howard St, SF"},
        {"location_num": 2, "name": "Ritual FiDi", "address": "49 Kearny St, SF"},
    ]

    all_classes = []
    for location in locations:
        info = requests.get(
            "https://reserve.ritualhotyoga.com/reserve/"
        ...
        for clas in schedule_blocks:
            ...
            if not "cancelled" in clas.text:
                ...
                clas_start = dateutil.parser.parse(
                    input_date.strftime("%m/%d/%Y")
                    + " "
                    + start_block.find(text=True).strip()
                ).astimezone(pst)
                ...
                instructor = clas.find_all("span", class_="scheduleInstruc")[
                    0
                ].text.strip()
