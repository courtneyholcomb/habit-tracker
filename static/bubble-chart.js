...
d3.queue().defer(d3.json, "/bubble-chart-data").await(ready);

function ready(error, datapoints) {
  // create scale for circle sizes
  let minData = d3.min(datapoints, (d) => d.units);
  let maxData = d3.max(datapoints, (d) => d.units);
  let sizeScale = d3.scaleLinear().domain([minData, maxData]).range([32, 70]);
  ...
  simulation.nodes(datapoints).on("tick", moveCircles);
